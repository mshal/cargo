...
enum Message<'a> {
    Run(String),
    BuildPlanMsg(String, ProcessBuilder, Arc<Vec<(PathBuf, Option<PathBuf>, TargetFileType)>>),
    Stdout(String),
    Stderr(String),
...
impl<'a> JobState<'a> {
    ...
    pub fn build_plan(&self, module_name: String, cmd: ProcessBuilder, filenames: Arc<Vec<(PathBuf, Option<PathBuf>, TargetFileType)>>) {
